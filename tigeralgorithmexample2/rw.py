...
class DetectionWriter:
    ...
    def write_detections(self, detections: List[tuple], spacing:tuple, x_offset: Union[int, float], y_offset: Union[int, float]):
        for detection in detections:
            x, y, probalility = detection[0].item(), detection[1].item(), detection[2].item()
            probalility = min(1.0, probalility)
            x += x_offset
            y += y_offset
    ...
    def add_point(self, x:  Union[int, float], y:  Union[int, float], probalility: float):
        ...
        # print('proba type:', probalility)
        # print('x:', x, type(x), 'x float:', float(x), type(float(x)))
        point = {"point": [float(x), float(y), _3d_space_value], "probability": float('%.4f' % probalility)}
